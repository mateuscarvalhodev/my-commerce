...
import { getShippingOptions } from "@/actions/shipping";
...
interface CartItemData {
  ...
  variant?: { id: string; name: string; price_delta?: number; options: Record<string, string> } | null;
}
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setAddresses(addrsResult as any);
      } catch {
            ...
            : paymentMethod === "boleto"
              ...
              : {
                  ...
                  card: {
                    ...
                    cvv: cardCvv,
                    billingAddress: {
                      line1: `${addresses.find((a) => a.id === selectedAddressId)?.number}, ${addresses.find((a) => a.id === selectedAddressId)?.street}`,
                      line2:
      ...
      <section className="space-y-6 rounded-2xl border bg-white p-5 shadow-sm">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <Button
            ...
            className="w-full"
            disabled={
              submitting || !selectedAddressId || !selectedShipping
            }
          >
            {submitting ? (
